...
const ManageListing = () => {
  ...
  const niches=[
    'lifestyle','fitness','food','travel','tech','gaming','fashion','beauty','business','education','entertainment','music','art','sport','health','finance','other'
  ]
  ...
  useEffect(()=>{
     if(!id|| !userListings.length) return;
     setIsEditing(true);
      ...
      <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8'>
       <div className='mb-8'>
          <h1 className='text-3xl font-bold text-gray-800'>
            {isEditing?" Edit listing ":"List you account"}
          </h1>
       ...
       <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <Section title='Account Metrics'>
              <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
                   <InputField label='Followers Count *' type='number' min={0}  value={formData.followers_count} placeholder='100000' onChange={(v)=>handleInputChange('followers_count',v)} required={true} />    
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
                    <InputField label='Primary Audience Country *' min={0}  value={formData.country} placeholder='United States' onChange={(v)=>handleInputChange('country',v)} />  
           ...
           <Section title='Screenshots & Proof'>
              ...
              {formData.images.length>0 && (
                <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mt-4'>
                      {formData.images.map((img,index)=>(
                         <div key={index} className='relative'>
                          ...
                          <button type='button' onClick={()=>removeImage(index)} className='absolute -top-2 -right-2 size-6 bg-red-600 text-white rounded-full hover:bg-red-700'>x</button>
                         </div>
                      ))}
                </div>
              )}
                ...
                <div className='flex justify-end gap-3 text-sm'>
                      <button onClick={()=>navigate(-1)} type='button' className='px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors'>Cancel
                      </button>
